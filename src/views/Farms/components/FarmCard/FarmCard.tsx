...
import { Flex, Text, Skeleton } from '@dinoswap/uikit'
...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, account }) => {
  ...
  const earnLabel = farm.dual ? farm.dual.earnLabel : 'DINO'
    ...
    <FCard>
      ...
      {!removed && (
        <Flex justifyContent="space-between" alignItems="center">
          ...
          <Text bold style={{ display: 'flex', alignItems: 'center' }}>
            { farm.apy? (
              <>
                <ApyButton lpLabel={lpLabel} addLiquidityUrl={farm.liquidityUrl || addLiquidityUrl} cakePrice={cakePrice} apy={farm.apy} />
                {farmAPY}%
              </>
            ) : (
              <>---%</>
      ...
      <ExpandingWrapper expanded={showExpandableSection}>
        <DetailsSection
          ...
          totalValueFormated={farm.liquidity.toNumber() && farm.liquidity.toNumber() > 0 ? totalValueFormated : "---"}
          lpLabel={lpLabel}
          addLiquidityUrl={farm.liquidityUrl ||addLiquidityUrl}
        />
      </ExpandingWrapper>
    </FCard>
  )
