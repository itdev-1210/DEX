...
import { useMasterchef, useSousChef, useJurassicPool } from './useContract'
...
export const useJurassicUnstake = (sousId) => {
  const dispatch = useDispatch()
  const { account } = useWeb3React()
  ...
  const handleUnstake = useCallback(
    async (amount: string, decimals: number) => {
      ...
      dispatch(updateUserPendingRewardJurassic(sousId, account))
    },
